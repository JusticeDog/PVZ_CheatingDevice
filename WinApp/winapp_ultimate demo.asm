...
change_tree_height	PROTO, :DWORD
change_adventure_level	PROTO, :DWORD
    ...
    .ELSEIF uMsg==WM_COMMAND 
        ...
        .IF lParam==0 
            ...
            .ELSEIF  ax==IDM_GETTEXT 
				...
				.IF bufferInt == 0
					invoke MessageBox,NULL,ADDR bufferempty,ADDR AppName,MB_OK 
					jmp dest
				.ENDIF
			...
			.ELSEIF  ax==IDM_GETTEXT_TREEH
				...
				mov eax, 1 ;For testing
				invoke	change_tree_height, Newtreeh
				.IF eax==1
			...
			.ELSEIF  ax==IDM_GETTEXT_ADVEN
				...
				invoke Int_Change,ADDR bufferInt
				.IF eax > 10
					invoke MessageBox,NULL,ADDR bufferNumAdven2,ADDR AppName,MB_OK 
				.ELSE
					add Newadven, eax
					.IF Newadven <= 51
						;invoke Change_adven, Newadven
						mov eax, 1 ;For testing
						invoke	change_adventure_level, Newadven
						.IF eax==1
...
; 冒险模式跳到指定关卡
change_adventure_level PROC,
				new_adventure_level_value:DWORD			; 冒险模式关卡的新数值
				local	base_addr:DWORD		; 存放基址
				local	hWnd_pro:DWORD		; 存放进程的句柄
				local	adventure_level_addr:DWORD		; 存放冒险模式关卡的地址
				local	success:DWORD	;获取基址和句柄是否成功
				local	adventure_level_value:DWORD		; 冒险模式关卡的当前值
				...
				.ENDIF
				.IF		success == 0	;获取失败
					jmp	quit_change_adventure_level
				.ENDIF
				;修改冒险模式关卡
				mov		esi,base_addr
				mov		adventure_level_addr, esi
				add		adventure_level_addr, 00331C50h			; adventure_level_addr 是冒险模式关卡基址
				; 01 读取内存,并且直接用读到的数值更新adventure_level_addr
				invoke	ReadProcessMemory, hWnd_pro, adventure_level_addr, addr adventure_level_addr, TYPE DWORD, 0
				mov		esi, adventure_level_addr
				add		esi, 94Ch
				mov		adventure_level_addr, esi
				; 02 读取内存,并且直接用读到的数值更新adventure_level_addr
				invoke	ReadProcessMemory, hWnd_pro, adventure_level_addr, addr adventure_level_addr, TYPE DWORD, 0
				mov		esi, adventure_level_addr
				add		esi, 50h
				mov		adventure_level_addr, esi
				; 03 读取内存,此时adventure_level_addr处的内容就是冒险模式关卡的数值
				invoke	ReadProcessMemory, hWnd_pro, adventure_level_addr, addr adventure_level_value, TYPE DWORD, 0
				; 04 写入内存
				; WriteProcessMemory(hpro, (LPVOID)adventure_level_addr, &new_adventure_level_value, 4, 0); //修改冒险模式关卡
				invoke	WriteProcessMemory, hWnd_pro, adventure_level_addr, addr new_adventure_level_value, TYPE DWORD, 0

quit_change_adventure_level:
				mov		eax, success		; 返回值设置为是否成功
				pop		edi
				pop		esi
				pop		ebx
				ret
change_adventure_level ENDP
